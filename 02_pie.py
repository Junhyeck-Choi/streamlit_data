...
st.title("날짜별 증상별 응급출동 비율")
st.caption("날짜를 선택하면 해당 날짜의 증상별 응급출동 비율을 원그래프로 보여줍니다.")
...
chart_data = chart_data.loc[chart_data["출동 건수"] > 0].copy()

total_calls = chart_data["출동 건수"].sum()
if total_calls <= 0:
    st.warning("선택한 날짜의 전체 출동 건수가 0이므로 비율을 계산할 수 없습니다.")
    st.stop()

chart_data["비율"] = chart_data["출동 건수"] / total_calls

figure = px.pie(
    chart_data,
    names="증상",
    values="출동 건수",
    title=f"{selected_date:%Y년 %m월 %d일} 증상별 응급출동 비율",
    hole=0.35,
)
...
figure.update_layout(
    legend_title_text="증상명",
    margin=dict(t=70, r=30, b=30, l=30),
)

st.plotly_chart(figure, use_container_width=True)
st.metric("선택 날짜의 전체 출동 건수", f"{total_calls:,.0f}건")
